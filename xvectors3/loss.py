...
class SoftmaxLoss(TopVirtualLoss):
    ...
    def forward(self, inputs, targets, entropy_regularization=0.0):
        ...
        if entropy_regularization > 0.00000001:
            #breakpoint()
            y_hat_logprob = F.log_softmax(outputs, dim=1)
            result -= entropy_regularization * (y_hat_logprob.exp() * y_hat_logprob).sum()
